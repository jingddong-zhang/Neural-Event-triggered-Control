...
def table(case):
    data = np.load('./data/lambda_{}.npy'.format(case),allow_pickle=True).item()
    data_num = data['num']
    data_num_inter = data['inter']
    data_mse = data['mse']
    for i in range(3):
        print('triggering times:',data_num[i-3].mean())
    for i in range(3):
        print('Inter-event time:',data_num_inter[i-3].mean())
    for i in range(3):
        print('MSE:',data_mse[i-3].mean())

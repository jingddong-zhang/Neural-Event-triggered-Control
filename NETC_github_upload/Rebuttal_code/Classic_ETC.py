...
class ETC_event(nn.Module):

    def __init__(self,S,Q,K,strength=0.5):
        super(ETC_event, self).__init__()
        self.sigma = 10.
        self.rho = 28.
        self.beta = 8 / 3
    ...
    def quad_qp(self,state):
        ...
        h = matrix(np.array([-x*self.sigma*(y-x)-y*(self.rho*x-y-x*z)-z*(x*y-self.beta*z)-(x**2+y**2+z**2)/(2*self.epi)],dtype=np.float64))
    ...
    def event_fn(self, t, state):
        ...
        mod_s = torch.sqrt(torch.sum(s**2))
        mod_e = torch.sqrt(torch.sum(e**2))
        g = self.gamma(mod_e)-self.alpha(mod_s)
        return g.to(device)
...
model = ETC_event(S,Q,K,0.5)


# test_times = torch.linspace(0, 2, 1000).to(device)
test_times = torch.linspace(0, 4, 1000).to(device) # we use this for rebuttal
...
for i in range(5):
    ...
    # if abs(trajectory_x[-1])<1.0:
        ...
        res[f'{i}'] = {'x': trajectory_x.numpy(), 'y': trajectory_y.numpy(), 'z': trajectory_z.numpy(),
                       'num': n_events, 'traj_events': traj_events}
np.save('./rebuttal/Classic_ETC', res)

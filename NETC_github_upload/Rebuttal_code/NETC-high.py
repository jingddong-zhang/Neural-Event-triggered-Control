...
def table_data():
    ...
    model = NETC_high(D_in, H1, D_out, (D_in,), [H1, 1],strength=0.1).to(device)
    model._control.load_state_dict(torch.load('./data/NETC-high_control.pkl'))
    ...
    for i in range(5):
        with torch.no_grad():
            ...
            res[f'{i}']={'x':trajectory_x.numpy(),'y':trajectory_y.numpy(),'z':trajectory_z.numpy(),'num':n_events,'traj_events':traj_events}
    np.save('./rebuttal/NETC_high',res)
    # plt.plot(test_times,trajectory_x)
    # plt.show()

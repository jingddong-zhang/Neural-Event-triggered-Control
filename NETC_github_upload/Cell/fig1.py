...
nlc =  torch.load('./data/fig1/nlc_event times_2000.pt')
nlc = np.array([_.detach().numpy() for _ in nlc])
nlc = nlc[np.where(nlc<=1.0)[0]]

netc =  torch.load('./data/fig1/netc_event times.pt')
...
nlc_cont = np.load('./data/fig1/nlc_control.npy')[:len(nlc)+1,0]
netc_cont = np.load('./data/fig1/netc_control.npy')[:len(netc)+1,0]
...
plt.legend(fontsize=ticksize,loc=1,frameon=True,labelcolor='k',handletextpad=0.3,borderpad=0.2,borderaxespad=0.3,handlelength=1.0)
...
ax1 = plt.subplot(133)
plot_grid()
nlc_traj = np.load('./data/fig1/nlc_traj_2000.npy')[:500]
netc_traj = np.load('./data/fig1/netc_traj.npy')[:500]
